...
body, .nocolor-black{ color: white; background-color: black; }
.nocolor-white{ color: black; background-color: white; }
.nocolor-gray { color: black; background-color: gray;  }
.spoiler.nocolor{ color: black; background-color: black; }
.hidden { font-size: 0px; }
a{ color: inherit !important; }

html, body { margin: 0px; padding: 0px; }
#headBar { position: fixed; width: 100%; border-bottom: 1px solid #FFF; background: inherit; 
            top: 0px; padding: 2 5; }
...
function fileCheck(date, first){
    ...
    .done(function(data) { 
        ...
        data = data.replace(/(\d{4}-\d{2}-\d{2}) /g,"");
        data = data.replace(/(\d{2}:\d{2}:\d{2})/g, addAnchor);
        //Give the JOIN and QUIT messages a span so they can be toggled
        data = data.replace(/JOIN/g,"<span class='connect'>JOIN</span>");
        data = data.replace(/QUIT/g,"<span class='connect'>QUIT</span>");
...
function goDate(){
    var toGo = $("#currentLog").val();
    
    if( toGo.length > 10 || toGo.length < 8){
    ...
    var newDay = Date.parse(toYear + "-" + toMonth + "-" + toDay);
...
$(document).ready(function(){
    ...
    $("#goDate").prop("disabled",false);
});

</script>
<nav id="headBar">
<input type="button" onclick="$('span').toggleClass('nocolor-'+currentColor);" value="Toggle Colors" /> 
<input type="button" onclick="$('.connect').parent().toggleClass('hidden');" value="Toggle Connections" />&nbsp; &nbsp; &nbsp; &nbsp;
<input type="button" onclick="changeDate('down');" value="<" /><input type=text id="currentLog" /><input type="button" id="goDate" onclick="goDate();" value="Go" /><input type="button" id="dateUp" onclick="changeDate('up');" value=">" />&nbsp; &nbsp; &nbsp; &nbsp;
